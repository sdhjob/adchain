...
#define AD_VM_OPEN_NAMESPACE namespace ad::vm {
#define AD_VM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "ad-vm parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "ad-vm memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "ad-vm system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "ad-vm auxiliary errors")
...
#define _AD_VM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define AD_VM_ASSERT_INVALIDATE(expr, err_type)                                                                       \
