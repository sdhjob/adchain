#include <ad/chain/protocol_feature_activation.hpp>
...
namespace ad { namespace chain {
   ...
   void protocol_feature_activation::reflector_init() {
      ...
      AD_ASSERT( protocol_features.size() > 0, ill_formed_protocol_feature_activation,
      ...
      for( const auto& d : protocol_features ) {
         auto res = s.insert( d );
         AD_ASSERT( res.second, ill_formed_protocol_feature_activation,
   ...
   )
   {
      ...
      AD_ASSERT( !enforce_disjoint || protocol_features.size() == expected_size,
