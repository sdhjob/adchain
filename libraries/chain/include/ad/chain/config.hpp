...
namespace ad { namespace chain { namespace config {
...
const static name system_account_name    { N(ad) };
const static name null_account_name      { N(ad.null) };
const static name producers_account_name { N(ad.prods) };
...
const static name ad_auth_scope       { N(ad.auth) };
const static name ad_all_scope        { N(ad.all) };
...
const static ad::chain::wasm_interface::vm_type default_wasm_runtime = ad::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace ad::chain::config

constexpr uint64_t AD_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / ad::chain::config::percent_100;
}

template<typename Number>
Number AD_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + ad::chain::config::percent_100 - ad::chain::config::percent_1)  / ad::chain::config::percent_100;
}
