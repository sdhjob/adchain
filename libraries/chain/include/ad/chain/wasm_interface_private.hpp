...
#include <ad/chain/webassembly/wavm.hpp>
#include <ad/chain/webassembly/wabt.hpp>
#ifdef AD_VM_OC_RUNTIME_ENABLED
#include <ad/chain/webassembly/ad-vm-oc.hpp>
#else
#define _REGISTER_ADVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
#include <ad/chain/webassembly/ad-vm.hpp>
#include <ad/chain/webassembly/runtime_interface.hpp>
...
using namespace fc;
using namespace ad::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
...
#ifdef AD_VM_OC_RUNTIME_ENABLED
      struct advmoc_tier {
         advmoc_tier(const boost::filesystem::path& d, const advmoc::config& c, const chainbase::database& db) : cc(d, c, db), exec(cc) {}
         advmoc::code_cache_async cc;
         advmoc::executor exec;
         advmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool advmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const advmoc::config& advmoc_config) : db(d), wasm_runtime_time(vm) {
         if(vm == wasm_interface::vm_type::wabt)
            runtime_interface = std::make_unique<webassembly::wabt_runtime::wabt_runtime>();
#ifdef AD_AD_VM_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ad_vm)
            runtime_interface = std::make_unique<webassembly::ad_vm_runtime::ad_vm_runtime<ad::vm::interpreter>>();
#endif
#ifdef AD_AD_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ad_vm_jit)
            runtime_interface = std::make_unique<webassembly::ad_vm_runtime::ad_vm_runtime<ad::vm::jit>>();
#endif
#ifdef AD_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ad_vm_oc)
            runtime_interface = std::make_unique<webassembly::advmoc::advmoc_runtime>(data_dir, advmoc_config, d);
#endif
         if(!runtime_interface)
            AD_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef AD_VM_OC_RUNTIME_ENABLED
         if(advmoc_tierup) {
            AD_ASSERT(vm != wasm_interface::vm_type::ad_vm_oc, wasm_exception, "You can't use AD VM OC as the base runtime when tier up is activated");
            advmoc.emplace(data_dir, advmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            AD_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            AD_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef AD_VM_OC_RUNTIME_ENABLED
         if(advmoc) for(auto it = first_it; it != last_it; it++)
            advmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  AD_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  AD_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#ifdef AD_VM_OC_RUNTIME_ENABLED
      fc::optional<advmoc_tier> advmoc;
#endif
   };
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_AD_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_ADVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, AD_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
