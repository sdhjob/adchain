...
namespace fc {
   ...
   void to_variant( const float128_t& f, variant& v ) {
      ...
      const ad::chain::uint128_t as_bytes = *reinterpret_cast<const ad::chain::uint128_t*>(&f);
      std::string s = "0x";	
   ...
   inline
   void to_variant( const ad::chain::shared_string& s, variant& v ) {
      v = variant(std::string(s.begin(), s.end()));
   }

   inline
   void from_variant( const variant& v, ad::chain::shared_string& s ) {
      string _s;
      from_variant(v, _s);
      s = ad::chain::shared_string(_s.begin(), _s.end(), s.get_allocator());
   }

   inline
   void to_variant( const ad::chain::shared_blob& b, variant& v ) {
      v = variant(base64_encode(b.data(), b.size()));
   }

   inline
   void from_variant( const variant& v, ad::chain::shared_blob& b ) {
      string _s = base64_decode(v.as_string());
      b = ad::chain::shared_blob(_s.begin(), _s.end(), b.get_allocator());
   }
   ...
   template<typename T>
   void to_variant( const ad::chain::shared_vector<T>& sv, variant& v ) {
      to_variant(std::vector<T>(sv.begin(), sv.end()), v);
   }

   template<typename T>
   void from_variant( const variant& v, ad::chain::shared_vector<T>& sv ) {
      std::vector<T> _v;
      from_variant(v, _v);
      sv = ad::chain::shared_vector<T>(_v.begin(), _v.end(), sv.get_allocator());
   }
}
...
DataStream& operator << ( DataStream& ds, const float128_t& v ) {
   fc::raw::pack(ds, *reinterpret_cast<const ad::chain::uint128_t*>(&v));
   return ds;
}

template<typename DataStream>
DataStream& operator >> ( DataStream& ds, float128_t& v ) {
   fc::raw::unpack(ds, *reinterpret_cast<ad::chain::uint128_t*>(&v));
   return ds;
}
