...
namespace fc {
  class variant;
  void to_variant(const ad::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, ad::chain::name& check);
} // fc
...
namespace ad::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, ad::chain::name& check);
...
#define N(X) ad::chain::string_to_name(#X)

} // ad::chain

namespace std {
   template<> struct hash<ad::chain::name> : private hash<uint64_t> {
      typedef ad::chain::name argument_type;
...
FC_REFLECT( ad::chain::name, (value) )
