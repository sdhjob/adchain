...
namespace wasm_constraints = ad::chain::wasm_constraints;
...
class ad_vm_instantiated_module : public wasm_instantiated_module_interface {
      using backend_t = backend<apply_context, Impl>;
   public:
      
      ad_vm_instantiated_module(ad_vm_runtime<Impl>* runtime, std::unique_ptr<backend_t> mod) :
         _runtime(runtime),
      ...
      void apply(apply_context& context) override {
         ...
         } catch(ad::vm::timeout_exception&) {
            context.trx_context.checktime();
         } catch(ad::vm::wasm_memory_exception& e) {
            FC_THROW_EXCEPTION(wasm_execution_error, "access violation");
         } catch(ad::vm::exception& e) {
   ...
   private:
      ad_vm_runtime<Impl>*            _runtime;
      std::unique_ptr<backend_t> _instantiated_module;
};

template<typename Impl>
ad_vm_runtime<Impl>::ad_vm_runtime() {}

template<typename Impl>
void ad_vm_runtime<Impl>::immediately_exit_currently_running_module() {
   throw wasm_exit{};
}

template<typename Impl>
bool ad_vm_runtime<Impl>::inject_module(IR::Module& module) {
   return false;
}

template<typename Impl>
std::unique_ptr<wasm_instantiated_module_interface> ad_vm_runtime<Impl>::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t>,
   ...
   try {
      ...
      return std::make_unique<ad_vm_instantiated_module<Impl>>(this, std::move(bkend));
   } catch(ad::vm::exception& e) {
      FC_THROW_EXCEPTION(wasm_execution_error, "Error building ad-vm interp: ${e}", ("e", e.what()));
   }
}

template class ad_vm_runtime<ad::vm::interpreter>;
template class ad_vm_runtime<ad::vm::jit>;
