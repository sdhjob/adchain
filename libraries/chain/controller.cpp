...
#include <ad/chain/transaction_context.hpp>

#include <ad/chain/block_log.hpp>
#include <ad/chain/fork_database.hpp>
#include <ad/chain/exceptions.hpp>

#include <ad/chain/account_object.hpp>
#include <ad/chain/code_object.hpp>
#include <ad/chain/block_summary_object.hpp>
#include <ad/chain/ad_contract.hpp>
#include <ad/chain/global_property_object.hpp>
#include <ad/chain/protocol_state_object.hpp>
#include <ad/chain/contract_table_objects.hpp>
#include <ad/chain/generated_transaction_object.hpp>
#include <ad/chain/transaction_object.hpp>
#include <ad/chain/reversible_block_object.hpp>
#include <ad/chain/genesis_intrinsics.hpp>
#include <ad/chain/whitelisted_intrinsics.hpp>
#include <ad/chain/database_header_object.hpp>

#include <ad/chain/protocol_feature_manager.hpp>
#include <ad/chain/authorization_manager.hpp>
#include <ad/chain/resource_limits.hpp>
#include <ad/chain/chain_snapshot.hpp>
#include <ad/chain/thread_utils.hpp>
#include <ad/chain/platform_timer.hpp>
...
struct controller_impl {
   ...
   void pop_block() {
      ...
      if( !prev ) {
         AD_ASSERT( fork_db.root()->id == head->header.previous, block_validate_exception, "attempt to pop beyond last irreversible block" );
         prev = fork_db.root();
      }
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         AD_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
      }
   ...
   inline void set_activation_handler() {
      ...
      AD_ASSERT( res.second, misc_exception, "attempting to set activation handler twice" );
   }
   ...
   :rnh(),
    ...
    wasmif( cfg.wasm_runtime, cfg.advmoc_tierup, db, cfg.state_dir, cfg.advmoc_config ),
    resource_limits( db ),
    authorization( s, db ),
...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( ad, ad, newaccount );
   SET_APP_HANDLER( ad, ad, setcode );
   SET_APP_HANDLER( ad, ad, setabi );
   SET_APP_HANDLER( ad, ad, updateauth );
   SET_APP_HANDLER( ad, ad, deleteauth );
   SET_APP_HANDLER( ad, ad, linkauth );
   SET_APP_HANDLER( ad, ad, unlinkauth );
/*
   SET_APP_HANDLER( ad, ad, postrecovery );
   SET_APP_HANDLER( ad, ad, passrecovery );
   SET_APP_HANDLER( ad, ad, vetorecovery );
*/

   SET_APP_HANDLER( ad, ad, canceldelay );
   }
   ...
   void log_irreversible() {
      AD_ASSERT( fork_db.root(), fork_database_exception, "fork database not properly initialized" );
      ...
      if( log_head ) {
         AD_ASSERT( root_id == blog.head_id(), fork_database_exception, "fork database root does not match block log head" );
      } else {
         AD_ASSERT( fork_db.root()->block_num == lib_num, fork_database_exception,
   ...
   void replay(std::function<bool()> shutdown) {
      ...
      if( start_block_num <= blog_head->block_num() ) {
         ...
         if( pending_head->block_num < head->block_num || head->block_num < fork_db.root()->block_num ) {
            ...
            AD_ASSERT( new_root, fork_database_exception, "unexpected error: could not find new LIB in fork database" );
   ...
   void startup(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      AD_ASSERT( snapshot, snapshot_exception, "No snapshot reader provided" );
      ...
      try {
         ...
         if( blog.head() ) {
            ...
            AD_ASSERT( lib_num > 0, snapshot_exception,
   ...
   void startup(std::function<bool()> shutdown, const genesis_state& genesis) {
      AD_ASSERT( db.revision() < 1, database_exception, "This version of controller::startup only works with a fresh state database." );
      const auto& genesis_chain_id = genesis.compute_chain_id();
      AD_ASSERT( genesis_chain_id == chain_id, chain_id_type_exception,
      ...
      if( blog.head() ) {
         AD_ASSERT( blog.first_block_num() == 1, block_log_exception,
   ...
   void startup(std::function<bool()> shutdown) {
      AD_ASSERT( db.revision() >= 1, database_exception, "This version of controller::startup does not work with a fresh state database." );
      AD_ASSERT( fork_db.head(), fork_database_exception, "No existing fork database despite existing chain state. Replay required." );

      uint32_t lib_num = fork_db.root()->block_num;
      auto first_block_num = blog.first_block_num();
      if( blog.head() ) {
         AD_ASSERT( first_block_num <= lib_num && lib_num <= blog.head()->block_num(),
                     block_log_exception,
   ...
   static auto validate_db_version( const chainbase::database& db ) {
      ...
      AD_ASSERT(header_idx.begin() != header_idx.end(), bad_database_version_exception,
   ...
   void init(std::function<bool()> shutdown) {
...

      {
         ...
         AD_ASSERT( state_chain_id == chain_id, chain_id_type_exception,
      ...
      AD_ASSERT( db.revision() >= head->block_num, fork_database_exception,
      ...
      if( read_mode == db_read_mode::IRREVERSIBLE ) {
         ...
         AD_ASSERT( itr == rbi.end() || itr->blocknum == lib_num + 1, reversible_blocks_exception,
         ...
         AD_ASSERT( head->block_num <= last_block_num, reversible_blocks_exception,
         ...
         if( ritr != rbi.rend()
            ...
            AD_ASSERT( rbitr != rbi.end(), reversible_blocks_exception, "pending head block not found in reversible blocks");
            auto rev_id = rbitr->get_block_id();
            AD_ASSERT( rev_id == pending_head->id,
   ...
   void read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      { /// load and upgrade the block header state
         ...
         AD_ASSERT( blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end,
                     block_log_exception,
      ...
      controller_index_set::walk_indices([this, &snapshot, &header]( auto utils ){
         ...
         if (std::is_same<value_t, global_property_object>::value) {
            ...
            if (std::clamp(header.version, v2::minimum_version, v2::maximum_version) == header.version ) {
               ...
               AD_ASSERT( genesis, snapshot_exception,
      ...
      AD_ASSERT( gpo.chain_id == chain_id, chain_id_type_exception,
   ...
   void create_native_account( const fc::time_point& initial_timestamp, account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            // The initial ad ABI value affects consensus; see  https://github.com/ad/ad/issues/7794
            // TODO: This doesn't charge RAM; a fix requires a consensus upgrade.
            a.abi.resize(sizeof(ad_abi_bin));
            memcpy(a.abi.data(), ad_abi_bin, sizeof(ad_abi_bin));
         }
      });
   ...
   transaction_trace_ptr push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline, uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false ) {
      ...
      AD_ASSERT( itr != idx.end(), unknown_transaction_exception, "unknown transaction" );
   ...
   { try {
      ...
      AD_ASSERT( gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
   ...
   const transaction_receipt& push_receipt( const T& trx, transaction_receipt_header::status_enum status,
      ...
      AD_ASSERT( net_usage_words*8 == net_usage, transaction_exception, "net_usage is not divisible by 8" );
   ...
   transaction_trace_ptr push_transaction( const transaction_metadata_ptr& trx,
                                           ...
                                           bool explicit_billed_cpu_time = false )
   {
      AD_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
      ...
      try {
         ...
         if( !explicit_billed_cpu_time ) {
            fc::microseconds already_consumed_time( AD_PERCENT(sig_cpu_usage.count(), conf.sig_cpu_bill_pct) );
   ...
   {
      AD_ASSERT( !pending, block_validate_exception, "pending block already exists" );
      ...
      if (!self.skip_db_sessions(s)) {
         AD_ASSERT( db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete )
      {
         ...
         if( new_protocol_feature_activations.size() > 0 ) {
            ...
            for( const auto& feature_digest : new_protocol_feature_activations ) {
               ...
               if( res.second ) {
                  ...
                  AD_ASSERT( !f.preactivation_required, protocol_feature_exception,
                              "attempted to activate protocol feature without prior required preactivation: ${digest}",
                              ("digest", feature_digest)
                  );
               } else {
                  AD_ASSERT( !res.first->second, block_validate_exception,
         ...
         AD_ASSERT( handled_all_preactivated_features, block_validate_exception,
         ...
         )
         {
            ...
            AD_ASSERT( gpo.proposed_schedule.version == pbhs.active_schedule_version + 1,
   ...
   void finalize_block()
   {
      AD_ASSERT( pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
      AD_ASSERT( pending->_block_stage.contains<building_block>(), block_validate_exception, "already called finalize_block");
      ...
      uint64_t CPU_TARGET = AD_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
      resource_limits.set_block_parameters(
         ...
         {AD_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, config::maximum_elastic_resource_multiplier, {99, 100}, {1000, 999}}
      );
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         AD_ASSERT( pending->_block_stage.contains<completed_block>(), block_validate_exception,
         ...
         if( add_to_fork_db ) {
            ...
            head = fork_db.head();
            AD_ASSERT( bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
      ...
      for( auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr ) {
         ...
         switch( status ) {
            case protocol_feature_set::recognized_t::unrecognized:
               AD_THROW( protocol_feature_exception,
            ...
            break;
            default:
               AD_THROW( protocol_feature_exception, "unexpected recognized_t status" );
            break;
         }

         AD_ASSERT( currently_activated_protocol_features.find( f ) == currently_activated_protocol_features.end(),
         ...
         AD_ASSERT( pfs.validate_dependencies( f, dependency_checker ), protocol_feature_exception,
...
#define AD_REPORT(DESC,A,B) \
   if( A != B ) { \
      elog("${desc}: ${bv} != ${abv}", ("desc", DESC)("bv", A)("abv", B)); \
   }

      AD_REPORT( "timestamp", b.timestamp, ab.timestamp )
      AD_REPORT( "producer", b.producer, ab.producer )
      AD_REPORT( "confirmed", b.confirmed, ab.confirmed )
      AD_REPORT( "previous", b.previous, ab.previous )
      AD_REPORT( "transaction_mroot", b.transaction_mroot, ab.transaction_mroot )
      AD_REPORT( "action_mroot", b.action_mroot, ab.action_mroot )
      AD_REPORT( "schedule_version", b.schedule_version, ab.schedule_version )
      AD_REPORT( "new_producers", b.new_producers, ab.new_producers )
      AD_REPORT( "header_extensions", b.header_extensions, ab.header_extensions )
...
#undef AD_REPORT
      ...
      try {
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            if( receipt.trx.contains<packed_transaction>() ) {
               ...
               AD_ASSERT( false, block_validate_exception, "encountered unexpected receipt type" );
            }
         ...
         if( producer_block_id != ab._id ) {
            ...
            AD_ASSERT( producer_block_id == ab._id, block_validate_exception, "Block ID does not match",
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      AD_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      AD_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block_header( b->previous );
      AD_ASSERT( prev, unlinkable_block_exception,
      ...
      return async_thread_pool( thread_pool.get_executor(), [b, prev, control=this]() {
         ...
         AD_ASSERT( b->transaction_mroot == trx_mroot, block_validate_exception,
                    ...
                    const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      AD_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         AD_ASSERT( b, block_validate_exception, "trying to push empty block" );
         AD_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
         ...
         if( s == controller::block_status::irreversible ) {
            ...
            AD_ASSERT( read_mode != db_read_mode::IRREVERSIBLE, block_validate_exception,
                            ...
                            const forked_branch_callback& forked_branch_cb, const trx_meta_cache_lookup& trx_lookup )
   {
      ...
      if( new_head->header.previous == head->id ) {
         ...
         if( branches.second.size() > 0 ) {
            ...
            AD_ASSERT( self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         AD_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         AD_ASSERT( !intersects, actor_blacklist_exception,
   ...
   void check_contract_list( account_name code )const {
      if( conf.contract_whitelist.size() > 0 ) {
         AD_ASSERT( conf.contract_whitelist.find( code ) != conf.contract_whitelist.end(),
                     contract_whitelist_exception,
                     "account '${code}' is not on the contract whitelist", ("code", code)
                   );
      } else if( conf.contract_blacklist.size() > 0 ) {
         AD_ASSERT( conf.contract_blacklist.find( code ) == conf.contract_blacklist.end(),
   ...
   void check_action_list( account_name code, action_name action )const {
      if( conf.action_blacklist.size() > 0 ) {
         AD_ASSERT( conf.action_blacklist.find( std::make_pair(code, action) ) == conf.action_blacklist.end(),
   ...
   void check_key_list( const public_key_type& key )const {
      if( conf.key_blacklist.size() > 0 ) {
         AD_ASSERT( conf.key_blacklist.find( key ) == conf.key_blacklist.end(),
   ...
   void validate_tapos( const transaction& trx )const {
      ...
      AD_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   switch( status ) {
      case protocol_feature_set::recognized_t::unrecognized:
         if( is_producing_block() ) {
            AD_THROW( subjective_block_production_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         } else {
            AD_THROW( protocol_feature_bad_block_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         }
      break;
      case protocol_feature_set::recognized_t::disabled:
      ...
      default:
         if( is_producing_block() ) {
            AD_THROW( subjective_block_production_exception, "unexpected recognized_t status" );
         } else {
            AD_THROW( protocol_feature_bad_block_exception, "unexpected recognized_t status" );
         }
      break;
   }
   ...
   // Even if they don't recognize the feature, the producer could change their nodead code to treat it like an
   ...
   AD_ASSERT( !is_protocol_feature_activated( feature_digest ),
               protocol_feature_exception,
   ...
   AD_ASSERT( std::find( pso.preactivated_protocol_features.begin(),
   ...
   AD_ASSERT( pfs.validate_dependencies( feature_digest, dependency_checker ),
               protocol_feature_exception,
...
void controller::start_block( block_timestamp_type when, uint16_t confirm_block_count )
{
   ...
   AD_ASSERT( !my->pending, block_validate_exception, "pending block already exists" );
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline, uint32_t billed_cpu_time_us ) {
   validate_db_available_size();
   AD_ASSERT( !in_immutable_mode(), transaction_type_exception, "push transaction not allowed in read-only mode" );
   AD_ASSERT( trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed" );
...
{
   AD_ASSERT( !in_immutable_mode(), transaction_type_exception, "push scheduled transaction not allowed in read-only mode" );
   validate_db_available_size();
...
void controller::set_action_blacklist( const flat_set< pair<account_name, action_name> >& new_action_blacklist ) {
   for (auto& act: new_action_blacklist) {
      AD_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
      AD_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
   }
...
time_point controller::pending_block_time()const {
   AD_ASSERT( my->pending, block_validate_exception, "no pending block" );
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   AD_ASSERT( BOOST_LIKELY( id != block_id_type() ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   AD_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
void controller::validate_expiration( const transaction& trx )const { try {
   ...
   AD_ASSERT( time_point(trx.expiration) >= pending_block_time(),
               expired_tx_exception,
               "transaction has expired, "
               "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
               ("trx.expiration",trx.expiration)("pending_block_time",pending_block_time()));
   AD_ASSERT( time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
void controller::validate_db_available_size() const {
   ...
   AD_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::validate_reversible_available_size() const {
   ...
   AD_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
void controller::set_greylist_limit( uint32_t limit ) {
   AD_ASSERT( 0 < limit && limit <= chain::config::maximum_elastic_resource_multiplier,
               misc_exception,
