...
protected:
    ...
    void TestEncodedOutputStream(const char* expectedFilename, bool putBOM) {
        // Test FileWriteStream
        {
            ...
            EncodedOutputStream<FileEncoding, FileWriteStream> ad(os, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, ad);
                EXPECT_TRUE(success);
            }
            ad.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            EncodedOutputStream<FileEncoding, MemoryBuffer> ad(mb, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, ad);
                EXPECT_TRUE(success);
    ...
    void TestAutoUTFOutputStream(UTFType type, bool putBOM, const char *expectedFilename) {
        // Test FileWriteStream
        {
            ...
            char buffer[16];
            FileWriteStream os(fp, buffer, sizeof(buffer));
            AutoUTFOutputStream<unsigned, FileWriteStream> ad(os, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, ad);
                EXPECT_TRUE(success);
            }
            ad.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            AutoUTFOutputStream<unsigned, MemoryBuffer> ad(mb, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, ad);
                EXPECT_TRUE(success);
