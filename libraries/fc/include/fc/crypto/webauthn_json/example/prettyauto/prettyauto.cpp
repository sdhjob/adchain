...
int main(int, char*[]) {
    ...
    OutputStream ad(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(ad);     // CHANGED
#else
    ...
    OutputStream ad(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(ad);             // CHANGED
#endif
