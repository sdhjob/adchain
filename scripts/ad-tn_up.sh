#!/bin/bash
#
# ad-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# ad-tn_bounce.sh and ad-tn_roll.sh scripts.

connected="0"

rundir=programs/nodead
prog=nodead
...
if [ "$PWD" != "$AD_HOME" ]; then
    echo $0 must only be run from $AD_HOME
    exit -1
fi

if [ ! -e $rundir/$prog ]; then
    echo unable to locate binary for nodead
    exit -1
fi
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/ad/node_$AD_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/ad/node_$AD_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
