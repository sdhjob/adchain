...
function usage() {
  ...
  -i DIR      Directory where ad is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the ad data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export AD_INSTALL_DIR=${INSTALL_LOCATION:-$AD_INSTALL_DIR}

if [[ ! -d "${AD_INSTALL_DIR}" ]]; then
   echo "[AD installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${AD_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/ad/ad/issues/6940
   echo "[AD installation found: ${AD_INSTALL_DIR}]" && INSTALL_PATHS+=("${AD_INSTALL_DIR}") # AD_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${AD_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/ad")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/ad")
            fi
            # Version < 1.8.0; Before we started using ~/ad/1.8.x
            ...
            break;;
         1 | false | [Nn]* ) echo " - Cancelled AD Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing AD Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/ad_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
